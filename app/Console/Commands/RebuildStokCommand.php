...
class RebuildStokCommand extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            try {
                ...
                $this->warn('⚠️  Backup gagal: ' . $e->getMessage());
            }
    ...
    private function collectTransaksi(): array
    {
        ...
        // Pembelian - PENTING: Pakai detailPembelian bukan detail!
        $pembelian = Pembelian::with('detailPembelian.barang.satuanKonversi')
        ...
        foreach ($pembelian as $p) {
            foreach ($p->detailPembelian as $d) {
                $transaksi[] = [
                    'type' => 'pembelian',
                    'id' => $p->id,
                    'detail' => $d,
        ...
        // Penjualan - PENTING: Pakai detailPenjualan bukan detail!
        $penjualan = Penjualan::with('detailPenjualan.barang.satuanKonversi')
        ...
        // Stok Opname - PENTING: Pakai detailStokOpname bukan detail!
        $stokOpname = StokOpname::with('detailStokOpname.barang.satuanKonversi')
        ...
        foreach ($stokOpname as $so) {
            foreach ($so->detailStokOpname as $d) {
    ...
    private function convertToBase(float $jumlah, string $satuan, Barang $barang): float
    {
        ...
        if (!$konversi) {
            Log::warning('⚠️  Satuan tidak ditemukan: ' . $satuan);
            return $jumlah;
        }

        return $jumlah * $konversi->jumlah_konversi;
    }

    private function getNomorReferensi(string $type, $parent): ?string
    {
        if ($type === 'pembelian') return $parent->nomor_pembelian;
        if ($type === 'penjualan') return $parent->nomor_nota;
        if ($type === 'stok_opname') return $parent->nomor_opname;
        return null;
    }
    ...
    private function showSummary()
    {
        ...
        $this->table(['Barang', 'Stok Akhir', 'Total Riwayat'], $rows);
    }
}