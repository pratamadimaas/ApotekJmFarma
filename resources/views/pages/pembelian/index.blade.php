                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle text-center text-sm">
                                        ...
                                        @endphp
                                        <span class="badge bg-gradient-{{ $statusClass }}">{{ ucfirst($item->status) }}</span>
                                    </td>
                                    
                                    {{-- Kolom Aksi yang Dirapikan --}}
                                    <td class="align-middle">
                                        {{-- 1. Tombol Detail --}}
                                        <a href="{{ route('pembelian.show', $item->id) }}" class="btn btn-sm btn-link text-secondary mb-0 p-0 me-2" title="Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        
                                        {{-- 2. Edit dan Hapus (Hanya untuk Admin) --}}
                                        @if(Auth::user()->role === 'admin') 
                                        
                                            <a href="{{ route('pembelian.edit', $item->id) }}" class="btn btn-sm btn-link text-info mb-0 p-0 me-2" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            
                                            <form action="{{ route('pembelian.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pembelian ini? Stok barang akan dikurangi/dikembalikan.');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-link text-danger mb-0 p-0" title="Hapus">
